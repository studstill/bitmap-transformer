...
var flip = require('./helpers/helpers').flipImageNP;
...
if (!cliArg || !(cliArg in options)) {
  err = 'Usage: node transformNP.js option\n' +
        'options: invert, grayscale, flip\n';
  throw err;
}

function transformNP(cliArg, options) {
  ...
  ee.on('writeFile', function(data) {
    fs.writeFile('lib/new-non-palette2.bmp', data, function(err) {
      if (err) throw err;
    });
...
transformNP(cliArg, options);

exports.transformNP = transformNP;
